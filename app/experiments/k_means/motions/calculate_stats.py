...
def calc_purities(parameter_sets, label_mapping):
    def calc_purity(tp_clustering, label_mapping):
        ...
        # filler_free_tp_clustering=tp_clustering[~np.isin(tp_clustering['cluster_id'],filler_cluster_ids)]
...
def calc_silhouette_scores(parameter_sets, data_definition):
    silhouette_scores = []
    for parameter_set in parameter_sets:
        clustering = parameter_set["clustering"]
        silhouette_score = metrics.silhouette_score(
            clustering[data_definition["features"]], clustering["cluster_id"]
        )
...
def extract_clustering_stats(parameter_sets, data_definition, label_mapping):
    ...
    sqs = np.array(calc_snapshot_qualities(parameter_sets, data_definition))
    tqs = np.array(calc_temporal_qualites(parameter_sets, data_definition))
    ...
    av_oq = oqs.mean()  # (av_sq+av_tqs)/2
    std_oq = statistics.stdev(oqs)
    av_purity = sum(purities) / len(purities)
    std_purity = statistics.stdev(purities)
    # ad_oq= (sqs-sqs.min())/(sqs.max()-sqs.min())+(tqs-tqs.min())/(tqs.max()-tqs.min()))
    return [av_sq, std_sq, av_tqs, std_tqs, av_oq, std_oq, av_purity, std_purity]
