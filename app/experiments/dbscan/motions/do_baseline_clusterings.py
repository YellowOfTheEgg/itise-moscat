...
def do_baseline_clustering(data, data_definition, eps_range, minpts_range):
...
if __name__ == "__main__":
    ...
    moscat = do_baseline_clustering(data, data_definition, eps_range, minpts_range)
    parameter_sets = list(
        map(lambda p: extract_dict_from_stepset(p), moscat.optimal_parameters)
    )
    moscat_clusterings.append({"w": 0, "parameter_sets": parameter_sets})

    # moscat_clusterings = run_clusterings(data, data_definition,init_point_set=init_point_set, max_iter_set=max_iter_set, k_set=k_set, weight_set=weight_set)

    output_dir = os.getcwd() + "/clusterings/"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    with open(output_dir + f"baseline_dbscan.pickle", "wb") as f:
